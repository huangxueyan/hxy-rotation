...
struct ResidualCostFunction
{
    ...
    bool operator()(const T* const ag_v_ang, T* residual) const  // ag_v_ang 8 vars of homography 
    {
        ...
        // approx method 
        {
            ...
            // points_later_T(0) += T(delta_time_later_) * vel(0) * N_norm.transpose() * points_T;
            // points_later_T(1) += T(delta_time_later_) * vel(1) * N_norm.transpose() * points_T;
            // points_later_T(2) += T(delta_time_later_) * vel(2) * N_norm.transpose() * points_T;
        }
        ...
        /* ceres interpolate version  */
        {
            ...
            residual[1] = T(yaml_regulization_factor_)*(ceres::pow(T(last_N_norm_(0)) - ag_v_ang[6], 2) + ceres::pow(T(last_N_norm_(1)) - ag_v_ang[7], 2));
...
void System::EstimateMotion_ransca_doublewarp_ceres(double ts_start, double ts_end, int sample_num, int total_iter_num)
{
    ...
    double residuals; 
    double ag_v_ang[8] = {est_angleAxis(0), est_angleAxis(1), est_angleAxis(2), 
                          est_trans_velocity(0), est_trans_velocity(1), est_trans_velocity(2),
                        est_N_norm(0), est_N_norm(1)};
    ...
    for(int iter_= 1; iter_<= total_iter_num; iter_++)
    {
        ...
        Eigen::Vector2d eg_N_norm(ag_v_ang[6],ag_v_ang[7]);
