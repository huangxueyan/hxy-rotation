...
void System::getWarpedEvent(EventBundle& eventIn, const Eigen::Vector3d & cur_ang_vel, EventBundle& event_out, bool later_warp)
{
    ...
    {    
        ...
        if (later_warp)
            vec_delta_time = eventIn.time_delta.array() - eventIn.time_delta(eventIn.size-1);   // to t1 
        else 
            vec_delta_time = eventIn.time_delta.array();  // to t0 
...
* \param samples_count 
*/
void System::getSampledVecUniform(vector<int>& vec_sampled_idx, int samples_count, double sample_start, double sample_end)
{
    cv::RNG rng(int(ros::Time::now().nsec));

    int occupied_num = 0;
    Eigen::Matrix<int, -1, -1> grid; 
    grid.resize(180, 240); 
    grid.setZero();
    ...
    for(int i=0; i< samples_count; i++)
    {
        ...
        if(count > yaml_denoise_num)  // TODO 
        {
            vec_sampled_idx.push_back(sample_idx);
            occupied_num++;
            grid(sampled_y, sampled_x)++;
        }
    }

    // std::cout << "occupied cell " << occupied_num << std::endl; 
