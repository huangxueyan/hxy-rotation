...
struct ResidualCostFunction
{
    ...
    bool operator()(const T* const ag, T* residual) const
    {
        ...
        /* get residual ceres interpolate version  */
        {
            ...
            interpolator_early_ptr_->Evaluate(points_2D_early_T(1) * T(2.0), points_2D_early_T(0) * T(2.0), &early_loss);
            interpolator_later_ptr_->Evaluate(points_2D_later_T(1) * T(2.0), points_2D_later_T(0) * T(2.0), &later_loss);
...
void System::EstimateMotion_ransca_ceres()
{
    ...
    for(int iter_= 1; iter_<= total_iter_num; iter_++)
    {
        ...
        cv::Mat cv_earlier_timesurface = cv::Mat(camera.height * 2, camera.width * 2, CV_32FC1); 
        cv::Mat cv_later_timesurface =   cv::Mat(camera.height * 2, camera.width * 2, CV_32FC1); 
        ...
        for(int i= event_warpped_Bundle.size*timesurface_range; i >=0; i--)
        {
            ...
            int sampled_x = std::round(event_warpped_Bundle.coord.col(i)[0] * 2);
            int sampled_y = std::round(event_warpped_Bundle.coord.col(i)[1] * 2); 
    ...
    t1 = ros::Time::now();
        ...
        int pixel_count = camera.width * camera.height * 4; // double resolution
    ...
    if(show_time_info)
        ...
        ceres::Grid2D<float,1> grid_early(line_grid_early.data(), 0, camera.height * 2, 0, camera.width * 2);
        ceres::Grid2D<float,1> grid_later(line_grid_later.data(), 0, camera.height * 2, 0, camera.width * 2);
        ...
        cv_earlier_timesurface_float_ = cv_earlier_timesurface.clone();
