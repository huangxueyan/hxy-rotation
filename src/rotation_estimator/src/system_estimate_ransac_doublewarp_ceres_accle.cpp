...
struct ResidualCostFunction
{
    ...
    bool operator()(const T* const ag, T* residual) const
    {
        ...
        int count = 0; 
        Eigen::Matrix<T, 3, 1> delta_points_T_early, delta_points_T_later, delta_second_points_T;
        ...
        {
            delta_points_T_early(0) = -(ag[2] + ag[5] * T(delta_time_early_))*T(points_(1)) + (ag[1] + ag[4] * T(delta_time_early_))*T(points_(2));  
            delta_points_T_early(1) =  (ag[2] + ag[5] * T(delta_time_early_))*T(points_(0)) - (ag[0] + ag[3] * T(delta_time_early_))*T(points_(2));
            delta_points_T_early(2) = -(ag[1] + ag[4] * T(delta_time_early_))*T(points_(0)) + (ag[0] + ag[3] * T(delta_time_early_))*T(points_(1));

            points_early_T(0) = T(points_(0)) + delta_points_T_early(0)*T(delta_time_early_) ;
            points_early_T(1) = T(points_(1)) + delta_points_T_early(1)*T(delta_time_early_) ;
            points_early_T(2) = T(points_(2)) + delta_points_T_early(2)*T(delta_time_early_) ;

            delta_points_T_later(0) = -(ag[2] + ag[5] * T(delta_time_later_))*T(points_(1)) + (ag[1] + ag[4] * T(delta_time_later_))*T(points_(2));  
            delta_points_T_later(1) =  (ag[2] + ag[5] * T(delta_time_later_))*T(points_(0)) - (ag[0] + ag[3] * T(delta_time_later_))*T(points_(2));
            delta_points_T_later(2) = -(ag[1] + ag[4] * T(delta_time_later_))*T(points_(0)) + (ag[0] + ag[3] * T(delta_time_later_))*T(points_(1));


            points_later_T(0) = T(points_(0)) + delta_points_T_later(0)*T(delta_time_later_) ;
            points_later_T(1) = T(points_(1)) + delta_points_T_later(1)*T(delta_time_later_) ;
            points_later_T(2) = T(points_(2)) + delta_points_T_later(2)*T(delta_time_later_) ;
        }
...
void System::EstimateMotion_ransca_ceres()
{
    ...
    double angleAxis[6] = {est_angleAxis(0), est_angleAxis(1), est_angleAxis(2), 0, 0, 0}; 
    
    for(int iter_= 1; iter_<= total_iter_num; iter_++)
    {
        // get timesurface earlier 
        Eigen::Vector<double, 6> eg_angleAxis(angleAxis);
        ...
        t1 = ros::Time::now();
        getWarpedEventImageAcc(eg_angleAxis, event_warpped_Bundle).convertTo(curr_warpped_event_image, CV_32FC3);  // get latest warpped events 
        t2 = ros::Time::now();
    ...
    if(show_time_info)
        ...
        getWarpedEventImageAcc(eg_angleAxis, event_warpped_Bundle, PlotOption::U16C1_EVNET_IMAGE, true);
